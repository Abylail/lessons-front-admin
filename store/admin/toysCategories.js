...
export const actions = {
  ...
  async fetchCategoryList({ commit, state }, optimize = false) {
    ...
    await this.$api.$get(`/api/v1/admin/toyCategory/get`)
      .then(({err, body}) => {
        if (!err) {
  ...
  async createCategory({ rootGetters, dispatch  }, categoryInfo) {
    await this.$api.$post(`/api/v1/admin/toyCategory/create`, categoryInfo)
      .then(({err, body}) => {
        if (!err) {
  ...
  async updateCategory({ rootGetters, dispatch  }, categoryInfo) {
    await this.$api.$put(`/api/v1/admin/toyCategory/update/${categoryInfo.id}`, categoryInfo)
      .then(({err, body}) => {
        if (!err) {
  ...
  async deleteCategory({ dispatch }, categoryInfo) {
    await this.$api.$delete(`/api/v1/admin/toyCategory/delete/${categoryInfo.id}`)
      .then(({err, body}) => {
        if (!err) {
